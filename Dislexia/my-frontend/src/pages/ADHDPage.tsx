...
interface ADHDResult {
  ...
  checklist: string[];
  timeEstimate: string;
  reformattedText: TextSubsection[]; // NEW: Full text broken into subsections
}
...
export default function ADHDPage() {
  ...
  const [showReformattedText, setShowReformattedText] = useState(false) // NEW: Toggle state

  // Helper function to parse bold markdown **text** into <strong> tags
  ...
  const generateADHDFriendlyText = async () => {
    ...
    setError('')
    setResult(null)
    setShowReformattedText(false) // Reset toggle when generating new content
...
`
      ...
      const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          temperature: 0.3,
          max_tokens: 2000, // Increased for reformattedText
        }),
      })
    ...
    <div className="disability-page adhd-page">
      ...
      <header className="page-header adhd-header">
        <div className="header-content">
          ...
          <p>Turn dense text into clear, actionable steps</p>
        </div>
      </header>
      ...
      <main className="adhd-content">
        <div className="layout-container">
          ...
          <div className="input-section">
            ...
            >
              {loading ? '⏳ Structuring...' : '✨ Make ADHD-Friendly'}
            </button>
                ...
                <div className="adhd-structured-content">
                  ...
                  {result.details && (
                    <section className="adhd-section details-section">
                      <h3>📖 Details (Optional Deep Dive)</h3>
                  ...
                  {/* Reformatted Full Text - Collapsible */}
                  {result.reformattedText && result.reformattedText.length > 0 && (
                    <section className="adhd-section fulltext-section">
                      <div className="fulltext-header">
                        <h3>📄 Full Text (Reformatted for Focus)</h3>
                        <button 
                          className="toggle-fulltext-button"
                          onClick={() => setShowReformattedText(!showReformattedText)}
                        >
                          {showReformattedText ? '▼ Hide Full Text' : '▶ Show Full Text'}
                        </button>
                      </div>
                      <p className="section-description">
                        The complete content, broken into bite-sized sections with one main idea each.
                      </p>
                      
                      {showReformattedText && (
                        <div className="reformatted-subsections">
                          {result.reformattedText.map((subsection, i) => (
                            <div key={i} className="text-subsection">
                              <h4>{parseBoldText(subsection.heading)}</h4>
